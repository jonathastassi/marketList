    ...
    register(req, res) {
        ...
        user.store()
            .then((result) => {
                ...
                res.status(201).json({'message': 'Novo usuário criado com sucesso!', 'data': userInsert});
            })
            .catch((err) => {
                res.status(500).json({'message': `Ocorreu um erro ao criar a conta de usuário! | ${err}`});
            });
    }
